...
When the local peer receives all recommendations, it computes reputation value $r_{i,j}$  as a weighted expected local experience ($ecb_{i,j}$, $eib_{i,j}$) from the remote peers with their remote experience ($er_{i,j}$).
\begin{equation}
\begin{split}
