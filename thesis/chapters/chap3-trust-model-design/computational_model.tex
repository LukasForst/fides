...
This section describes how Fides to whom and how much it can trust to other remote peers.
...
The first one is that we want to robustly capture average behavior of the peers. In order to do that, we will be trying to capture average behavior of the peers and then approximating the standard deviations from said behavior and normalizing them.
...
As outlined previously, service trust $st_{i, j}$ is a value that describes how much does peer $i$ trusts that remote peer $j$ will provide a \textit{good service}.
We compute the $st_{i, j}$ in equation \ref{eq:service-trust} by weighting local experience with peer's $j$ service, with the reputation $j$ got from the network, when it was first seen by $i$.
...
\todo[inline]{We need to justify why this Integrity belief. Is i in SORT? if yes just reference. If not, then we need to justify why is not like a normal stdev. Is the more weird for me}
...
\todo{Do we need to explain why did we added all these properties to a single recommendation? Original paper does that, but I'm not sure if we need to do that or not. (sebas: well, since you did it, we need to explain. Now don't delete)}
