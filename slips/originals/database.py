...
class Database(object):
    ...
    def start(self, config):
        ...
        if not hasattr(self, 'r'):
            try:
                ...
                print(
                    '[DB] Error in database.py: Is redis database running? You can run it as: "redis-server --daemonize yes"')
    ...
    def getTWofTime(self, profileid, time):
        ...
        try:
            data = \
            self.r.zrangebyscore('tws' + profileid, float('-inf'), float(time), withscores=True, start=0, num=-1)[-1]
        except IndexError:
    ...
    def addNewOlderTW(self, profileid, startoftw):
        try:
            ...
            self.outputqueue.put(
                '04|database|[DB]: Created and added to DB the new older TW with id {}. Time: {} '.format(twid,
                                                                                                          startoftw))
    ...
    def addNewTW(self, profileid, startoftw):
        try:
            ...
            self.r.zadd('tws' + profileid, data)
            self.outputqueue.put(
                '04|database|[DB]: Created and added to DB for profile {} on TW with id {}. Time: {} '.format(profileid,
                                                                                                              twid,
                                                                                                              startoftw))
            # The creation of a TW now does not imply that it was modified. You need to put data to mark is at modified
            return twid
    ...
    def add_ips(self, profileid, twid, ip_as_obj, columns, role: str):
        ...
        try:
            ...
            self.print('add_ips(): As a {}, add the {} IP {} to profile {}, twid {}'.format(role, type_host_key,
                                                                                            str(ip_as_obj), profileid,
                                                                                            twid), 3, 0)
    ...
    def add_tuple(self, profileid, twid, tupleid, data_tuple, role, starttime, uid):
        ...
        try:
            self.print(
                'Add_tuple called with profileid {}, twid {}, tupleid {}, data {}'.format(profileid, twid, tupleid,
                                                                                          data_tuple), 3, 0)
            ...
            try:
                ...
                # There was no previous data stored in the DB
                self.print(
                    'First time for tuple {} as an {} for {} in TW {}'.format(tupleid, direction, profileid, twid), 3,
                    0)
    ...
    def add_port(self, profileid: str, twid: str, ip_address: str, columns: dict, role: str, port_type: str):
        ...
        try:
            ...
            try:
                ...
                self.print('add_port(): Adding this new info about port {} for {}. Key: {}. NewData: {}'.format(port,
                                                                                                                profileid,
                                                                                                                key_name,
                                                                                                                innerdata),
                           3, 0)
                ...
                prev_data[port] = innerdata
                self.print(
                    'add_port(): First time for port {} for {}. Key: {}. Data: {}'.format(port, profileid, key_name,
                                                                                          innerdata), 3, 0)
            # self.outputqueue.put('01|database|[DB] {} '.format(ip_address))
            # Convet the dictionary to json
            data = json.dumps(prev_data)
            self.print(
                'add_port(): Storing info about port {} for {}. Key: {}. Data: {}'.format(port, profileid, key_name,
                                                                                          prev_data), 3, 0)
            # Store this data in the profile hash
    ...
    def add_out_dns(self, profileid, twid, stime, flowtype, uid, query, qclass_name, qtype_name, rcode_name, answers,
                    ttls):
        """
    ...
    def get_default_gateway(self):
        ...
        if not stored_gateway:
            ...
            if platform.system() == "Darwin":
                ...
                route_default_result = re.findall(r"([\w.][\w.]*'?\w?)",
    ...
    def getDataFromProfileTW(self, profileid: str, twid: str, direction: str, state: str, protocol: str, role: str,
                             type_data: str) -> dict:
        """
        ...
        try:
            self.print(
                'Asked to get data from profile {}, {}, {}, {}, {}, {}, {}'.format(profileid, twid, direction, state,
                                                                                   protocol, role, type_data), 3, 0)
            ...
            value = {}
            if data:
                self.print('Key: {}. Getting info for Profile {} TW {}. Data: {}'.format(key, profileid, twid, data), 3,
                           0)
                # Convert the dictionary to json
                portdata = json.loads(data)
                value = portdata
